...
export function applyRecordEdits(trip: Trip, operations: AssistantOperation[]): Trip {
  ...
  for (const op of operations) {
    ...
    const keys: Record<typeof op.entity, string[]> = { trip: ["title","subtitle","startDate","endDate"], city: ["name","englishName","country","note","startDate","endDate","appleGuideUrl"], day: ["date","title"], ticket: ["title","provider","date","time","meta","code","passengers","departureTime","arrivalTime","arrivalDate","checkInDate","checkOutDate","checkInTime","checkOutTime","includesBreakfast"], journal: ["title","date","text"] };
    ...
    if ("appleGuideUrl" in op.changes && op.changes.appleGuideUrl && !/^https:\/\/(?:maps\\.apple\\.com|guides\\.apple\\.com)\//i.test(String(op.changes.appleGuideUrl))) throw new Error("请提供有效 Apple 地图指南链接");
    let found = false;
