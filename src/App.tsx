    ...
    updateTrip((current) => {
      ...
      operations.forEach((operation) => {
        if (operation.type === "add_city") {
          ...
          next = { ...next, cities: [...next.cities, { id: uid("city"), name: raw.name, englishName: raw.englishName || raw.name, dates: raw.dates || "待安排", note: raw.note || "给这座城市留一点偶遇。", color: raw.color || "#e4ddcf", days }] };
        }
...
function TicketsView({ trip, onAdd, onEdit, onRemove }: { trip: Trip; onAdd: () => void; onEdit: (id: string) => void; onRemove: (id: string) => void }) {
  ...
  return <section className="tickets-page">
    ...
    {layout === "cards" ? <div className="ticket-grid">{tickets.map((ticket) => <TicketCard key={ticket.id} ticket={ticket} city={trip.cities.find((city) => city.id === ticket.cityId)} onEdit={() => onEdit(ticket.id)} onRemove={() => onRemove(ticket.id)} onPreview={() => setPreview(ticket)} />)}</div> : <div className="ticket-list">{tickets.map((ticket) => { const city = trip.cities.find((item) => item.id === ticket.cityId); return <article key={ticket.id}><time>{ticket.date || "待定"}</time><span className="ticket-list-kind">{ticket.kind}</span><div><strong>{ticket.title}</strong><small>{ticket.provider} · {city?.name || "旅程"}</small></div><p>{ticket.time}<small>{ticket.meta}</small></p><div className="ticket-list-actions">{ticket.image && <button onClick={() => setPreview(ticket)}>▣</button>}<button onClick={() => onEdit(ticket.id)}>✎</button><button onClick={() => onRemove(ticket.id)}>×</button></div></article>; })}</div>}
    ...
    {preview?.image && <div className="ticket-image-lightbox" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setPreview(null)}><section role="dialog" aria-modal="true" aria-label={`${preview.title}完整图片`}><button onClick={() => setPreview(null)} aria-label="关闭完整图片">×</button><img src={preview.image} alt={`${preview.title}完整票据`} /><strong>{preview.title}</strong></section></div>}
  </section>;
}
