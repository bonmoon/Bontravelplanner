...
function TicketsView({ trip, onAdd, onEdit, onRemove }: { trip: Trip; onAdd: () => void; onEdit: (id: string) => void; onRemove: (id: string) => void }) {
  ...
  return <section className="tickets-page">
    ...
    {layout === "cards" ? <div className="ticket-grid">{tickets.map((ticket) => <TicketCard key={ticket.id} ticket={ticket} city={trip.cities.find((city) => city.id === ticket.cityId)} onEdit={() => onEdit(ticket.id)} onRemove={() => onRemove(ticket.id)} />)}</div> : <div className="ticket-list">{tickets.map((ticket) => { const city = trip.cities.find((item) => item.id === ticket.cityId); return <article key={ticket.id}><time>{ticket.date || "待定"}</time><span className="ticket-list-kind">{ticket.kind}</span><div><strong>{ticket.title}</strong><small>{ticket.provider} · {city?.name || "旅程"}</small></div><p>{ticket.time}<small>{ticket.meta}</small></p><div className="ticket-list-actions"><button onClick={() => onEdit(ticket.id)}>✎</button><button onClick={() => onRemove(ticket.id)}>×</button></div></article>; })}</div>}
